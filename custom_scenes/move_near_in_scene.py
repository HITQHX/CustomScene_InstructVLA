...
def _default_move_near_xy_configs(n_objects: int):
    candidate_configs = [
        [[-0.10, 0.30], [-0.10, 0.00], [-0.15, 0.15]],
        [[-0.15, 0.20], [-0.15, 0.00], [-0.05, 0.15]],
        [[-0.10, 0.25], [-0.10, 0.10], [-0.15, 0.05]],
    ]
    if n_objects < 2:
        raise ValueError("MoveNear requires at least two objects.")
    if n_objects > len(candidate_configs[0]):
        raise ValueError(
            f"Default MoveNear configs only support up to {len(candidate_configs[0])} objects."
        )
    return [config[:n_objects] for config in candidate_configs]


def _default_move_near_quat_configs(n_objects: int):
    return [[[1.0, 0.0, 0.0, 0.0] for _ in range(n_objects)]]
...
@register_env("MoveNearCustomInScene-v0", max_episode_steps=80)
class MoveNearCustomInSceneEnv(MoveNearInSceneEnv, CustomOtherObjectsInSceneEnv):
    def __init__(
        self,
        source_obj_name: str = None,
        target_obj_name: str = None,
        distractor_obj_names: Optional[List[str]] = None,
        xy_configs: Optional[List[List[List[float]]]] = None,
        quat_configs: Optional[List[List[List[float]]]] = None,
        **kwargs,
    ):
        self._source_obj_name = source_obj_name
        self._target_obj_name = target_obj_name
        self._distractor_obj_names = list(distractor_obj_names or [])
        self._xy_configs = xy_configs
        self._quat_configs = quat_configs
        super().__init__(**kwargs)

    def _resolve_default_model_ids(self):
        model_ids = [self._source_obj_name, self._target_obj_name]
        model_ids.extend(self._distractor_obj_names)
        model_ids = [model_id for model_id in model_ids if model_id is not None]
        if len(model_ids) < 2:
            raise ValueError(
                "MoveNearCustomInSceneEnv requires at least source and target objects."
            )
        return model_ids

    def _resolve_object_id(self, model_ids, preferred_name, fallback_id):
        if preferred_name is not None and preferred_name in model_ids:
            return model_ids.index(preferred_name)
        return fallback_id

    def _resolve_xy_configs(self, n_objects):
        if self._xy_configs is not None:
            return self._xy_configs
        return _default_move_near_xy_configs(n_objects)

    def _resolve_quat_configs(self, n_objects):
        if self._quat_configs is not None:
            return self._quat_configs
        return _default_move_near_quat_configs(n_objects)
    ...
    def reset(self, seed=None, options=None):
        ...
        model_ids = options.get("model_ids", None)
        if model_ids is None:
            model_ids = self._resolve_default_model_ids()
        ...
        xy_configs = self._resolve_xy_configs(len(model_ids))
        quat_configs = self._resolve_quat_configs(len(model_ids))
        episode_id = obj_init_options.get(
            "episode_id",
            self._episode_rng.randint(len(xy_configs) * len(quat_configs)),
        )

        if "source_obj_id" not in obj_init_options:
            obj_init_options["source_obj_id"] = self._resolve_object_id(
                model_ids, self._source_obj_name, 0
            )
        if "target_obj_id" not in obj_init_options:
            target_fallback = 1 if len(model_ids) > 1 else 0
            obj_init_options["target_obj_id"] = self._resolve_object_id(
                model_ids, self._target_obj_name, target_fallback
            )
        if "init_xys" not in obj_init_options:
            obj_init_options["init_xys"] = xy_configs[
                (episode_id % (len(xy_configs) * len(quat_configs))) // len(quat_configs)
            ]
        if "init_rot_quats" not in obj_init_options:
            obj_init_options["init_rot_quats"] = quat_configs[
                episode_id % len(quat_configs)
            ]
